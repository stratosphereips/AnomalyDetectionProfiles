...
class DashboardTests(unittest.TestCase):
    ...
    def test_dashboard_html_is_full_view_and_has_required_tabs(self):
        ...
        self.assertIn("experimental_pca_mode", dashboard.SETTING_METADATA)
        self.assertIn("experimental_isolation_mode", dashboard.SETTING_METADATA)
        self.assertIn("experimental_rarity_mode", dashboard.SETTING_METADATA)
        self.assertIn("experimental_change_mode", dashboard.SETTING_METADATA)
        self.assertIn("experimental_graph_mode", dashboard.SETTING_METADATA)
        ...
        self.assertIn("Global benign training interval", html)
        self.assertIn("summary?.training_start", html)
        self.assertIn("it never restarts for a protocol", html)
        ...
        self.assertIn("Pipeline validation", html)
        self.assertIn("renderPipelineStatus", html)
        self.assertIn('<details class="detector-details">', html)
        self.assertNotIn('<details class="detector-details" open>', html)
        self.assertIn('id="detectorStatusSummary"', html)
        self.assertIn('Core active · ${modeCount("active")}', html)
        self.assertIn("Pipeline comparison", html)
        self.assertIn("renderPipelineComparison", html)
        self.assertIn("Detector findings", html)
        self.assertIn('id="methodCount"', html)
        self.assertIn("renderDetectorFindings", html)
        self.assertIn("detectorFindingEntries", html)
        self.assertIn("filteredDetectorFindingEntries", html)
        self.assertIn('["log","comparison"].includes(state.tab)', html)
        self.assertIn("sourceFindings", html)
        self.assertIn("destinationFindings", html)
        self.assertIn("Source-IP communication graph", html)
        self.assertIn("Destination-IP window", html)
        self.assertIn("Official effect", html)
        self.assertIn("Core overlap", html)
        self.assertIn("Responsible identifiers", html)
        self.assertIn("candidate.responsible_flows", html)
        self.assertIn("No representative flow records were available", html)
        self.assertIn("SHADOW · comparison only", html)
        self.assertIn("Pipeline validation checks — not detectors", html)
        self.assertIn("bounded display score", html)
    ...
    def test_learning_guide_is_self_contained_and_served_by_dashboard(self):
        ...
        self.assertIn("How the detector decides whether a feature value is anomalous", guide)
        self.assertIn("Feature and feature value are different", guide)
        self.assertIn("feature definition: flow_count", guide)
        self.assertIn("current feature value: flow_count = 47", guide)
        self.assertNotIn("Full example: a sudden DNS burst", guide)
        self.assertNotIn('id="walkthrough"', guide)
        self.assertIn("11. Settings that change detection", guide)
        ...
        self.assertIn("Scoring a window and adapting the model are two separate decisions", guide)
        self.assertIn("no feature crossed its threshold", guide)
        self.assertIn("suspicious_alpha", guide)
        self.assertIn("Noise floor: what it is and why it is needed", guide)
        self.assertIn("smallest normal variation", guide)
        self.assertIn("candidate_floor = max(0.01, residual q10", guide)
        self.assertIn("new_noise_floor = 0.95 × old_noise_floor", guide)
        self.assertIn("How Zeek Anomaly Detection Works", guide)
        self.assertIn("<h2>Summary</h2>", guide)
        ...
        self.assertIn("10. Evidence behind core statistical anomalies", guide)
        self.assertIn("still part of the core statistical detector", guide)
        self.assertNotIn("Why did the detector alert?", guide)
        self.assertIn("Source-IP/protocol/window anomaly", guide)
        self.assertIn("JSON <code>event</code>", guide)
        self.assertIn("older names for compatibility", guide)
        self.assertIn("Optional detectors and operating modes", guide)
        self.assertIn("core anomalies ∪ candidates from Active optional detectors", guide)
        self.assertIn("Same-run comparison", guide)
        self.assertIn("core_mirror_v1", guide)
        self.assertIn("decision agreement = |overlap|", guide)
        self.assertIn("Additional detection methods", guide)
        self.assertIn("D² = δᵀ (C*)⁻¹ δ", guide)
        self.assertIn("PCA reconstruction", guide)
        self.assertIn("Isolation Forest", guide)
        self.assertIn("Empirical rarity and novelty", guide)
        self.assertIn("Time-series level change", guide)
        self.assertIn("Communication graph behavior", guide)
        self.assertIn("Exploring individual findings", guide)
        self.assertIn("Open <strong>Detector findings</strong>", guide)
        self.assertIn("representative Zeek flow records", guide)
        self.assertIn("same search, source-IP, destination-IP, and protocol filters", guide)
        self.assertIn("What is a z-score?", guide)
        self.assertIn("Interactive z-score plot", guide)
        self.assertIn("signed z-score = (current value − historical mean)", guide)
        self.assertIn("updateZPlot", guide)
        self.assertIn("These are two different views, not two names for the same thing", guide)
        self.assertIn("One record, two aggregations", guide)
        self.assertIn("Destination-IP windows: a different fixed feature list", guide)
        self.assertIn("incoming_flow_count", guide)
        self.assertLess(guide.index('id="ensemble"'), guide.index('id="multivariate"'))
        self.assertLess(guide.index('id="settings"'), guide.index('id="experimental"'))
        self.assertLess(guide.index('id="experimental"'), guide.index('id="multivariate"'))
        self.assertIn("Simple definition:", guide)
        self.assertNotIn("dispersion", guide.lower())
        self.assertNotIn("Interactive noise-floor calculation", guide)
        self.assertNotIn("updateFloorCalculator", guide)
        self.assertNotIn("bucket", guide.lower())
        self.assertIn("why transform every value with ln(1+x)?", guide)
        self.assertIn("Why add 1?", guide)
        self.assertIn("ln((1+b)/(1+a))", guide)
        self.assertIn("global grouping key = (IP, window_start)", guide)
        self.assertIn("source-IP/protocol/window anomaly", guide)
        self.assertIn("IP@window-start", guide)
        self.assertIn("They therefore form two different possible global-IP events", guide)
        self.assertIn("effective_minimum_components", guide)
